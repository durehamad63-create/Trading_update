    ...
    async def chart_websocket(websocket: WebSocket, symbol: str):
        ...
        try:
            while True:
                ...
                # Always use 5 steps for all timeframes
                num_steps = 5
                
                # Always use multi-step predictor for 5 steps
                try:
                    from multistep_predictor import multistep_predictor
                    if multistep_predictor:
                        multistep_data = await multistep_predictor.get_multistep_forecast(symbol, model_timeframe)
                        if multistep_data:
